...
export const StockAnalysisModule: React.FC<StockAnalysisModuleProps> = ({
  ...
  if (isLoading) {
    return (
      <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '12px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '11px', color: '#6B7280' }}>{t?.analyzingMarket || 'Analyzing market...'}</p>
      </div>
    );
  }
  ...
  if (!data || !data.symbol) {
    return (
      <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '8px', textAlign: 'center' }}>
        <Activity size={20} strokeWidth={1} style={{ color: '#D1D5DB', margin: '0 auto' }} />
      </div>
    );
  }
    ...
    if (active && payload && payload.length) {
      return (
        <div style={{ 
          ...
          <p style={{ fontSize: '7px', color: '#6B7280', margin: 0 }}>{label}</p>
          <p style={{ fontSize: '9px', fontWeight: 'bold', color: '#1F2937', margin: '1px 0 0 0' }}>${payload[0].value.toFixed(2)}</p>
        </div>
      );
    }
    return null;
  ...
  return (
    <div style={{ maxWidth: '100%', overflowX: 'hidden' }}>
      ...
      {/* SECTION 1: GLOBAL MARKET INDICES - More compact */}
      <div style={{ backgroundColor: '#FEF08A', borderRadius: '8px', padding: '6px', marginBottom: '6px' }}>
        ...
        <div style={{ display: 'flex', gap: '4px', marginBottom: '4px' }}>
          {row1Indices.map((index, i) => (
            <div key={i} style={{ flex: 1, backgroundColor: 'white', borderRadius: '4px', padding: '3px', textAlign: 'center' }}>
              <p style={{ fontSize: '7px', fontWeight: 'bold', color: '#4B5563', margin: '1px 0' }}>{index.name}</p>
              <p style={{ fontSize: '9px', fontWeight: 'bold', color: '#1F2937', margin: '1px 0' }}>{index.value}</p>
              <p style={{ fontSize: '6px', fontWeight: 'bold', color: index.positive ? '#10B981' : '#EF4444', margin: '1px 0' }}>{index.change}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px', flexWrap: 'wrap', gap: '4px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={{ fontSize: '9px', fontWeight: 'bold', color: '#1F2937', margin: 0 }}>{currentPrice}</p>
            </div>
            <div style={{ textAlign: 'center' }}>
              <p style={{ fontSize: '6px', color: '#6B7280', margin: 0 }}>RSI</p>
              <p style={{ fontSize: '9px', fontWeight: 'bold', color: '#3B82F6', margin: 0 }}>{rsiValue}</p>
            </div>
            <div style={{ textAlign: 'center' }}>
              <p style={{ fontSize: '6px', color: '#6B7280', margin: 0 }}>MACD</p>
              <p style={{ fontSize: '9px', fontWeight: 'bold', color: '#10B981', margin: 0 }}>{macdValue}</p>
            </div>
          </div>
        </div>
        
        {/* Chart - Smaller height */}
        <div style={{ width: '100%', height: isMobile ? '90px' : '140px' }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 2, right: 2, left: 2, bottom: 2 }}>
              ...
              <Line type="monotone" dataKey="price" stroke="#3B82F6" strokeWidth={1.5} dot={false} activeDot={{ r: 2 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      ...
      {/* SECTION 3: STOCK INFORMATION - More compact */}
      <div style={{ backgroundColor: '#FEF08A', borderRadius: '8px', padding: '6px', marginBottom: '6px' }}>
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          <div style={{ flex: 1, backgroundColor: 'white', borderRadius: '4px', padding: '3px', textAlign: 'center' }}>
            <p style={{ fontSize: '5px', color: '#6B7280', margin: '1px 0' }}>{langKey === 'English' ? 'Market Cap' : '市值'}</p>
            <p style={{ fontSize: '8px', fontWeight: 'bold', color: '#1F2937', margin: '1px 0' }}>{marketCap}</p>
          </div>
          <div style={{ flex: 1, backgroundColor: 'white', borderRadius: '4px', padding: '3px', textAlign: 'center' }}>
            <p style={{ fontSize: '5px', color: '#6B7280', margin: '1px 0' }}>{langKey === 'English' ? 'P/E Ratio' : '市盈率'}</p>
            <p style={{ fontSize: '8px', fontWeight: 'bold', color: '#1F2937', margin: '1px 0' }}>{peRatio}</p>
          </div>
          <div style={{ flex: 1, backgroundColor: 'white', borderRadius: '4px', padding: '3px', textAlign: 'center' }}>
            <p style={{ fontSize: '5px', color: '#6B7280', margin: '1px 0' }}>{langKey === 'English' ? 'Volume' : '成交量'}</p>
            <p style={{ fontSize: '8px', fontWeight: 'bold', color: '#1F2937', margin: '1px 0' }}>{typeof volume === 'number' ? (volume / 1000000).toFixed(1) + 'M' : volume}</p>
          </div>
        </div>
      </div>

      {/* SECTION 4: AI ANALYSIS TEXT - More compact */}
      <div style={{ backgroundColor: '#1F2937', borderRadius: '8px', padding: '6px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            <div style={{ display: 'flex', gap: '-4px' }}>
              <div style={{ width: '20px', height: '20px', borderRadius: '50%', backgroundColor: 'white', border: '1.5px solid #3B82F6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '8px', color: '#3B82F6' }}>M</div>
              <div style={{ width: '20px', height: '20px', borderRadius: '50%', backgroundColor: '#374151', border: '1.5px solid #3B82F6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '8px', color: 'white' }}>T</div>
            </div>
            <div>
        ...
        <div style={{ backgroundColor: '#374151', borderRadius: '6px', padding: '6px' }}>
          {summaryText ? (
            ...
            <div style={{ textAlign: 'center', padding: '8px' }}>
              ...
              <p style={{ fontSize: '9px', color: '#9CA3AF' }}>{langKey === 'English' ? 'Waiting for analysis...' : langKey === 'Cantonese' ? '等待分析結果...' : '等待分析结果...'}</p>
            </div>
          )}
        </div>
