...
interface AuthModalProps {
  ...
  onClose: () => void;
  onSuccess?: () => void;
  langKey?: string;  // Add langKey prop
}

export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onSuccess, langKey = 'English' }) => {
  ...
  const [tempUserData, setTempUserData] = useState<{ email: string; displayName: string; userId: string } | null>(null);
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Store temp user data with userId and show terms modal
      setTempUserData({ 
        email, 
  ...
  const handleTermsClose = () => {
    setShowTerms(false);
    setTempUserData(null);
    // Sign out the user if they declined
  ...
  return (
    <>
      ...
      {/* TermsModal - placed outside the main modal */}
      {showTerms && tempUserData && (
        <TermsModal
          isOpen={showTerms}
