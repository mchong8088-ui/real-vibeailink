...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      console.log("✅ Signup successful, showing terms");
  ...
  const handleTermsAccept = async () => {
    ...
    if (!error) {
      console.log("✅ Terms accepted, profile updated");
      setShowTerms(false);
      onClose();
      if (onSuccess) onSuccess();
      window.location.reload();
    ...
    <>
      <div style={{ position: 'fixed', inset: 0, zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.5)' }}>
        <div style={{ width: '100%', maxWidth: '400px', margin: '16px' }}>
          <div style={{ backgroundColor: 'white', borderRadius: '24px', boxShadow: '0 4px 20px rgba(0,0,0,0.1)', padding: '24px', position: 'relative' }}>
            ...
            <h2 style={{ fontSize: '20px', fontWeight: 'bold', textAlign: 'center', color: '#111827', marginBottom: '20px' }}>
            ...
            <form onSubmit={isLogin ? handleLogin : handleSignUp} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {!isLogin && (
                <input
                  ...
                  style={{ width: '100%', padding: '12px', fontSize: '14px', backgroundColor: '#F9FAFB', borderRadius: '12px', border: 'none', outline: 'none' }}
                />
              )}
              <input
                type="email"
            ...
            <div style={{ marginTop: '16px' }}>
              <button
                ...
                style={{ width: '100%', backgroundColor: 'transparent', color: '#2563EB', fontWeight: '500', padding: '8px', borderRadius: '12px', border: 'none', cursor: 'pointer', fontSize: '13px' }}
              >
      ...
      {showTerms && tempUser && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div style={{ backgroundColor: 'white', borderRadius: '16px', maxWidth: '500px', width: '90%', maxHeight: '80vh', overflow: 'auto' }}>
            <div style={{ padding: '20px' }}>
              <h2 style={{ fontSize: '18px', fontWeight: 'bold', marginBottom: '16px' }}>Terms of Service</h2>
              <div style={{ marginBottom: '20px', fontSize: '13px', lineHeight: '1.5' }}>
                <p>Please read and accept our terms of service to continue.</p>
                <p style={{ marginTop: '12px' }}>By accepting, you agree to our Terms of Service and Privacy Policy.</p>
              </div>
              <button
                onClick={handleTermsAccept}
                disabled={loading}
                style={{ width: '100%', backgroundColor: '#22C55E', color: 'white', padding: '12px', borderRadius: '8px', border: 'none', cursor: 'pointer' }}
              >
